...
function ColorFilter({ data, color_dropdown }) {
  ...
  return (
    <>
      <div
        className={`ddd w-[100%] lg:w-[320px] border-[#fbf1e8] border-[1px] relative lg:absolute pt-[20px] lg:pt-0 z-[2] bg-[#fff] f-out max-h-[350px] overflow-auto`}
        ref={color_dropdown}
      >
        <div className="ddd w-[100%] px-[20px] py-[20px]">
          <div className="ddd mt-[10px]">
              ...
              <>
                {data.group1.map((value, index) => {
                  return (
                    <div
                      key={value.colorCode}
                      className={`ddd ${value.colorCode} w-[100%] relative h-[48px] p-[10px]`}
                      onClick={() => addColor(value, data.group1, "group1")}
                    >
                      <p className="ddd fwl text-[#fff] text-[11px] capitalize">
                        {value.name}
                      </p>

                      {activeInGroup1 == index ? (
                        <i
                          className="ddd las la-times text-[#fff] text-[11px] absolute top-[10px] right-[10px]  "
                          // onClick={() => openSidebar()}
                        ></i>
                      ) : (
                        ""
            ...
            {data.group2 ? <div className="ddd w-[100%] mt-[40px]"></div> : ""}
              ...
              <>
                {data.group2.map((value, index) => {
                  return (
                    <div
                      key={value.colorCode}
                      className={`ddd ${value.colorCode} w-[100%] bbb h-[48px] p-[10px]`}
